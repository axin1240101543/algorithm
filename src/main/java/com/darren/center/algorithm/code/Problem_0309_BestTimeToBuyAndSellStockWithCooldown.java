...
public class Problem_0309_BestTimeToBuyAndSellStockWithCooldown {
    ...
    public static void main(String[] args) {
        Solution solution = new Problem_0309_BestTimeToBuyAndSellStockWithCooldown().new Solution();
    }
    ...
    class Solution {
        ...
        public int maxProfit(int[] prices) {
            ...
            // buy[i]值的含义是：max{ 所有可能性[之前交易获得的最大收益 - 最后buy动作的收购价格] }
            int[] buy = new int[n];
            // sell[i] :0...i范围上，最后一次操作是sell动作，
            // 这最后一次操作有可能发生在i位置，也可能发生在之前
            // sell[i]值的含义：0...i范围上，最后一次动作是sell的情况下，最好的收益
            int[] sell = new int[n];
            buy[1] = Math.max(-prices[0], -prices[1]);
            sell[1] = Math.max(0, prices[1] - prices[0]);
            for (int i = 2; i < n; i++){
                //如果i位置发生了买的行为 那么i-1位置是冷冻期
                buy[i] = Math.max(buy[i - 1], sell[i - 2] - prices[i]);
                //如果i位置发生了sell行为，那么我们一定要找到 {之前获得尽可能好的收益 - 最后一次的收购价格尽可能低}，
                sell[i] = Math.max(sell[i - 1], buy[i - 1] + prices[i]);
            }
